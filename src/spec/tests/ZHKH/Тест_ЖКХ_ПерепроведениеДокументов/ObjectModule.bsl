...
Процедура ЗаполнитьНаборТестов(НаборТестов) Экспорт
	...
	ОписанияТестов.Добавить("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерерасчетИзЗаКачества");
	ОписанияТестов.Добавить("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерераспределениеОплаты");
...
Процедура ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерерасчетИзЗаКачества() Экспорт
	
	СписокДокументов = ПолучитьСписокДокументов_ПерерасчетИзЗаКачества();	
	ТабДокДвиженийДо = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерерасчетИзЗаКачества_СКД", "ПерерасчетИзЗаКачества_До.XLSX");
	ОтменитьДвиженияДокументовПоСписку();
	ПерепровестиВсеДокументыПоСпискуДокументов_ПерерасчетИзЗаКачества();
	ТабДокДвиженийПосле = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерерасчетИзЗаКачества_СКД", "ПерерасчетИзЗаКачества_После.XLSX");
	СравнитьТабличныеДокументыДвижений("Перерасчет из-за качества");
	
КонецПроцедуры

Процедура ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_ПерераспределениеОплаты() Экспорт
	
	СписокДокументов = ПолучитьСписокДокументов_ПерераспределениеОплаты();	
	ТабДокДвиженийДо = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_Оплата_Задолженность_СКД", "ПерераспределениеОплаты_Задолженность_До.XLSX");
	ТабДокДвиженийДо_Оплата = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_Оплата_Оплата_СКД", "ПерераспределениеОплаты_Оплата_До.XLSX");
	ОтменитьДвиженияДокументовПоСписку();
	ПерепровестиВсеДокументыПоСпискуДокументов_ПерераспределениеОплаты();
	ТабДокДвиженийПосле = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_Оплата_Задолженность_СКД", "ПерераспределениеОплаты_Задолженность_После.XLSX");
	ТабДокДвиженийПосле_Оплата = ПолучитьТабДокументДвижений("ТестДолжен_ПерепровестиИСравнитьДвиженияДокументов_Оплата_Оплата_СКД", "ПерераспределениеОплаты_Оплата_После.XLSX");
	СравнитьТабличныеДокументыДвижений("ПерераспределениеОплаты_Задолженность");
	ТабДокДвиженийДо = ТабДокДвиженийДо_Оплата;
	ТабДокДвиженийПосле = ТабДокДвиженийПосле_Оплата;
	СравнитьТабличныеДокументыДвижений("ПерераспределениеОплаты_Оплата");
...
Процедура ПерепровестиВсеДокументыПоСпискуДокументов_Начисления()
	...
	Для каждого ТекДок Из СписокДокументов Цикл
		...
		ТЗ = ДокОб.СоставНачислений.Выгрузить(, "ВидРасчета");
		ТЗ.Свернуть("ВидРасчета");
		МассивВРДокумента = ТЗ.ВыгрузитьКолонку("ВидРасчета");
...
Процедура ПерепровестиВсеДокументыПоСпискуДокументов_ЗаполнениеПриНепредПоказаний_НормативноеКолвоИСреднемес()
	...
	Для каждого ТекДок Из СписокДокументов Цикл
		...
		СтруктураПараметров.Вставить("ФлагУчитыватьВсеПоказанияДляСреднего", Истина);
		СтруктураПараметров.Вставить("ФлагОтрицательныеПоказанияНеУчитыватьДляСреднего", Ложь);
		...
		СтруктураПараметров.Вставить("МаксимальноеКоличествоПериодовРасчетаСреднемесячныхНачислений", 6);
		СтруктураПараметров.Вставить("МинимальноеКоличествоПериодовРаботыСчетчикаДляСреднемесячных", 0);
...
Процедура ПерепровестиВсеДокументыПоСпискуДокументов_Оплата()
	...
	//для Волжского установим константу
	Если Константы.ркУчитыватьПриОплатеТекущийПериод.Получить()Тогда
		Константы.ркУчитыватьПриОплатеТекущийПериод.Установить(Ложь);
	КонецЕсли;
...
#Область ДокументовПерерасчетИзЗаКачества

Функция ПолучитьСписокДокументов_ПерерасчетИзЗаКачества()
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("текУчетныйМесяц", текУчетныйМесяц);
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ркПерерасчетИзЗаКачестваОтсутствияУслуг.Ссылка КАК пДокумент
		|ИЗ
		|	Документ.ркПерерасчетИзЗаКачестваОтсутствияУслуг КАК ркПерерасчетИзЗаКачестваОтсутствияУслуг
		|ГДЕ
		|	ркПерерасчетИзЗаКачестваОтсутствияУслуг.Проведен
		|	И ркПерерасчетИзЗаКачестваОтсутствияУслуг.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&текУчетныйМесяц, МЕСЯЦ) И КОНЕЦПЕРИОДА(&текУчетныйМесяц, МЕСЯЦ)";
...
Процедура ПерепровестиВсеДокументыПоСпискуДокументов_ПерерасчетИзЗаКачества()

	Для каждого ТекДок Из СписокДокументов Цикл
	
		ДокОб = ТекДок.ПолучитьОбъект();
		Документы.ркПерерасчетИзЗаКачестваОтсутствияУслуг.РасчитатьНачисления(ДокОб);
		ДокОб.Записать(РежимЗаписиДокумента.Проведение);
...
#Область ПерераспределениеОплаты

Функция ПолучитьСписокДокументов_ПерераспределениеОплаты()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	рцПерераспределениеОплаты.Ссылка КАК пДокумент
		|ИЗ
		|	Документ.рцПерераспределениеОплаты КАК рцПерераспределениеОплаты
		|ГДЕ
		|	рцПерераспределениеОплаты.Проведен
		|	И рцПерераспределениеОплаты.Дата МЕЖДУ НАЧАЛОПЕРИОДА(&текУчетныйМесяц, МЕСЯЦ) И КОНЕЦПЕРИОДА(&текУчетныйМесяц, МЕСЯЦ)
		|
		|УПОРЯДОЧИТЬ ПО
		|	рцПерераспределениеОплаты.Номер";
...
Процедура ПерепровестиВсеДокументыПоСпискуДокументов_ПерераспределениеОплаты()

	Для каждого ТекДок Из СписокДокументов Цикл
	
		ДокОб = ТекДок.ПолучитьОбъект();
		ПерераспределеннаяОплатаТЗ = Документы.рцПерераспределениеОплаты.ПолучитьПерераспределениеОплаты(
			ДокОб.Дата,ДокОб.УК,ДокОб.РаспределятьМеждуУК,ДокОб.РаспределятьМеждуПоставщикам,ДокОб.РаспределятьМеждуВидамиРасчетов);

		ДокОб.ПерераспределеннаяОплата.Загрузить(ПерераспределеннаяОплатаТЗ);
		ДокОб.Записать(РежимЗаписиДокумента.Проведение);
